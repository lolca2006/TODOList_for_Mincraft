...
public class InventoryResourceOverlay {
    ...
    public static void render(GuiGraphics graphics, int screenWidth, int screenHeight, int mouseX, int mouseY,
        ...
        if (!Services.CONFIG.inventoryOverlayEnabled()) return;
        ...
        Font font = mc.font;
        int maxItems = Services.CONFIG.inventoryOverlayMaxItems();
        ...
        // Position to the right of the inventory GUI with a small gap
        int panelX = guiRight + 6;
        int panelY = guiTop;

        // If JEI overlay is visible on the right, try to fit between inventory and JEI
        if (JEIHelper.isJEIOverlayVisible()) {
            int jeiLeftEdge = screenWidth - 170;
            if (panelX + panelWidth > jeiLeftEdge) {
                // Not enough space on the right - move to the left of the inventory
                panelX = guiLeft - panelWidth - 6;
            }
        }

        // If panel goes off the right edge, move to the left of inventory
        if (panelX + panelWidth > screenWidth - 4) {
            panelX = guiLeft - panelWidth - 6;
        }

        // If panel goes off the left edge, clamp to 2
        if (panelX < 2) {
            panelX = 2;
        }
